...
BMT_list = set()
for row in dt_list:
	time_value = time.mktime(time.strptime(row[1], '%m/%d/%Y %I:%M:%S %p'))
	row.append(time_value)
	BM_list.add((row[0],row[2]))
	BMT_list.add((time_value, row[0], row[2]))
...
for row in dt_list:
	...
	cur_BM = (row[0], row[2])
	cur_BMT = (row[4], row[0], row[2])
	
	if cur_time > prev_time:
		seen_BM  = set()
		# new period, cnt + 1, and assign to ordered_BMT_dic 
		BM_dic[cur_BM][1] += 1
		ordered_BMT_dic[cur_BMT][2] = BM_dic[cur_BM][1]

		# add cur_BM to seen_BM
		seen_BM.add(cur_BM)

		# update all the accumulated values because of the new period.
	...
	# add up the value for this period
	ordered_BMT_dic[cur_BMT][0] += int(row[3])

	# assign the accumulated value to this period
